...
import { ArrowLeft, Briefcase, Building2, Clock3, LogIn, Phone, UserRound, Users } from 'lucide-react';
...
function InsideCard({ visitor, onCheckout }: { visitor: Visitor; onCheckout: () => void }) { return <article className="visitor-card"><div className="visitor-card-top"><div className="person-line"><span className="person-avatar large">{visitor.visitorName.slice(0, 1).toUpperCase()}</span><div><h2>{visitor.visitorName}</h2><span>{visitor.visitorCode}</span></div></div><StatusBadge status="INSIDE" /></div><div className="visitor-info"><span><Phone size={15} />{visitor.phoneNumber}</span>{visitor.companyName && <span><Building2 size={15} />{visitor.companyName}</span>}{visitor.department && <span><Briefcase size={15} />{visitor.department}</span>}<span><LogIn size={15} />{format(visitor.checkInAt)}</span><span><UserRound size={15} />{visitor.personToMeet || 'No host specified'}</span></div><div className="visitor-reason"><small>REASON FOR VISIT</small><strong>{visitor.reasonForVisit}</strong></div><button className="checkout-btn" onClick={onCheckout}>Check out visitor <span>→</span></button></article>; }
